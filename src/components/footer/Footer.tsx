...
const Footer = () => {
    return (
        <>
            <section className="relative mt-20 pt-24 pb-28 bg-orange-50 overflow-hidden">
                <div className="relative z-10 container px-4 mx-auto w-full max-w-screen-xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 -m-8">
                        <div className="w-full p-8">
                            ...
                            <Link href={contact.location} target='_blank' className="flex lg:py-3 items-start gap-1 font-medium leading-relaxed md:max-w-xs mt-7">
                                <div className='w-20'>
                            ...
                            <div className='flex items-center gap-2'>
                                ...
                                <Link href={links.location} target='_blank' className='pt-3 flex items-center gap-2 group'>
                                    <div className='bg-gray-200  hover:bg-gray-300 cursor-pointer transition-all duration-200 rounded-full w-10 h-10 p-3 flex justify-center items-center'>
                                        <SlLocationPin />
                                    </div>
                                </Link>
                                            ...
                                            <ul>
                                                {
                                                    page?.children?.map((child) => {
                                                        return <li className="mb-3.5 text-sm"><Link className="text-heading hover:underline font-medium leading-relaxed" href={child?.url}>{child?.title}</Link></li>
                                                    })
                                                }
                                            </ul>
                                        </div >
                        ...
                        <div className="w-full p-8">
                            ...
                            <Link href={`tel:${contact.phoneNumber1}`} className='flex items-center gap-3 pb-2 group'>
                                <div className='flex gap-3 items-center'>
                                    <LuPhoneCall className='hover:!text-primary transition-all duration-200' />
                                    <p className='text-sm text-heading font-medium leading-relaxed group-hover:text-primary transition-all duration-200'>{contact.phoneNumber1}</p>
                                </div>
                            </Link>
